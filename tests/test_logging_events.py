...
def _install_test_logger(
    monkeypatch, ensure_return: str = "123e4567-e89b-12d3-a456-426614174000"
):
    logger = DummyLogger()
...
def test_delete_conversation_logs_state(monkeypatch):
    ...
    assert fake_ensure.calls == [None]
    assert any(
        name == "conversation_delete_requested" for _level, name, _ in logger.records
    )
...
def test_export_and_import_handlers_log(monkeypatch, tmp_path):
    ...
    assert any(
        name == "conversation_export_started" for _level, name, _ in logger.records
    )
    assert any(
        name == "conversation_export_completed" for _level, name, _ in logger.records
    )

    history_path = tmp_path / "history.json"
    history_path.write_text(
        json.dumps([{"role": "assistant", "content": "hello"}]), encoding="utf-8"
    )
    ...
    assert any(
        name == "conversation_import_started" for _level, name, _ in logger.records
    )
    assert any(
        name == "conversation_import_completed" for _level, name, _ in logger.records
    )
